...
const Skills: React.FC = () => {
  ...
  const value1 =
    typeof window !== "undefined" && window.innerWidth < 768 ? 28 : 48;
  const value2 =
    typeof window !== "undefined" && window.innerWidth < 768 ? 28 : 40;
  return (
    <section className="relative w-full bg-black py-8 md:py-12 overflow-hidden md:min-h-screen">
      <BackgroundRippleEffect className="absolute inset-0 -z-10 pointer-events-none" />

      <div
        ref={containerRef}
        className="relative z-10 max-w-6xl mx-auto px-4 flex flex-col items-center gap-6 md:gap-12"
      >
        <h2 className="text-center text-2xl sm:text-3xl md:text-4xl font-bold text-white">
          Skills & Services
          <p className="text-base"> I can provide</p>
        </h2>

        {skills.map((skill, idx) => (
          <div
            key={idx}
            className="flex flex-row items-center justify-center w-full skill-video-wrapper"
            data-index={idx}
          >
            <motion.p
              className="text-2xl sm:text-4xl md:text-7xl lg:text-8xl font-extrabold text-white text-right mr-1 sm:mr-2 w-auto whitespace-nowrap"
              initial={{ x: 0 }}
              animate={{ x: visibleVideos[idx] ? -value : 0 }}
            ...
            <motion.div
              className="rounded-2xl sm:rounded-3xl md:rounded-4xl border border-white/40 w-[100px] h-[65px] sm:w-[140px] sm:h-[95px] md:w-[180px] md:h-[120px] lg:w-[220px] lg:h-[140px] overflow-hidden flex items-center justify-center -mx-8 sm:-mx-12 md:-mx-16 lg:-mx-20"
            ...
            <motion.p
              className="text-2xl sm:text-4xl md:text-7xl lg:text-8xl font-extrabold text-white text-left ml-1 sm:ml-2 w-auto whitespace-nowrap"
              initial={{ x: 0 }}
              animate={{ x: visibleVideos[idx] ? value : 0 }}
              transition={{ duration: 1.2, ease: "easeOut" }}
            >
        ...
        <LogoLoop
          className="mt-4 md:mt-7 -mb-2 md:-mb-5"
          logos={techLogos}
          speed={60}
          direction="right"
          logoHeight={value1}
